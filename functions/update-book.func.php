...
if (!function_exists('updateBookWithSynopsys')) 
{
    function updateBookWithSynopsys($title,$isbn,$publish_at,$writer_id,$price,$synopsys,$id)
    {
        //global $db;
        $db = SinglePDO::getInstance();
        
        $stmt = $db->prepare("UPDATE book SET title = :title, isbn = :isbn, publish_at = :publish_at, writer_id = :writer_id, price = :price, synopsys = :synopsys WHERE id = :id");
        $stmt->bindValue(':title', $title, PDO::PARAM_STR);       
        $stmt->bindValue(':isbn', $isbn, PDO::PARAM_STR);       
        $stmt->bindValue(':publish_at', $publish_at, PDO::PARAM_STR);       
        $stmt->bindValue(':writer_id', $writer_id, PDO::PARAM_INT);       
